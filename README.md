...
[![Version](https://img.shields.io/badge/version-2.0.0-14b8a6)](#)
...
- **服务**（只读）：枚举所有 Windows 服务的名称、显示名、状态、类型。
- **启动项**（只读）：枚举开机启动程序的名称、命令、来源（当前用户 / 所有用户 / 启动文件夹）。
- **速启指令 / 系统托盘 / 明暗主题**。

## 安全说明

PortCheck 里的「结束进程」不是关闭某一个端口或单个连接，而是结束占用它的进程。
