...
// 每个账号一行：左侧圆环显示 5 小时窗口已用百分比（颜色分级），右侧账号名 +
// 「重置倒计时 · 周百分比」。数据自取：启动查询一次 + 每 5 分钟轮询；
// 主窗口增删账号时通过 codingplan:changed 事件即时刷新。
// 窗口高度由后端按行数调整（SetUsageRows），行数 = min(账号数,3) + 溢出提示行。
...
const ringOffset = (q: CodingPlanQuota | null) =>
  C * (1 - (q ? Math.min(100, Math.max(0, q.usedPercent)) : 0) / 100)
const ringText = (q: CodingPlanQuota | null) => (q ? `${Math.round(q.usedPercent)}` : '—')
...
const line2 = (u: CodingPlanUsage | null | undefined) => {
  ...
  if (u.fiveHour) {
    ...
    const wk = u.weekly ? `周 ${Math.round(u.weekly.usedPercent)}%` : '无周限额'
    return `${cd ? `重置 ${cd} · ` : ''}${wk}`
  }
  if (u.weekly) {
